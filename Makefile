# This Makefile is for the PGObject::Simple extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/PGObject/Simple.pm]
#     AUTHOR => [q[Chris Travers <chris.travers@gmail.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
#     NAME => q[PGObject::Simple]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], PGObject=>q[1.1] }
#     VERSION_FROM => q[lib/PGObject/Simple.pm]
#     clean => { FILES=>q[PGObject-Simple-*] }
...
FULL_AR = /usr/bin/ar
LD = gcc
LDDLFLAGS = -shared -O2 -g -pipe -Wall -Wp,-D_FORTIFY_SOURCE=2 -fexceptions -fstack-protector --param=ssp-buffer-size=4 -m32 -march=i686 -mtune=atom -fasynchronous-unwind-tables -Wl,-z,relro  -L/usr/local/lib
...
MAKE = make
FULLEXT = PGObject/Simple
BASEEXT = Simple
PARENT_NAME = PGObject
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/PGObject/Simple.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/PGObject
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/PGObject
...
TO_INST_PM = lib/PGObject/Simple.pm

PM_TO_BLIB = lib/PGObject/Simple.pm \
...
DISTNAME = PGObject-Simple
DISTVNAME = PGObject-Simple-1.3
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/PGObject/Simple.pm $(INST_MAN3DIR)/PGObject::Simple.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: "Minimalist stored procedure mapper based on LedgerSMB'\''s DBObject"' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Chris Travers <chris.travers@gmail.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: PGObject-Simple' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  PGObject: 1.1' >> META_new.yml
	$(NOECHO) $(ECHO) '  Test::More: 0' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: 1.3' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Minimalist stored procedure mapper based on LedgerSMB'\''s DBObject",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Chris Travers <chris.travers@gmail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "PGObject-Simple",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "PGObject" : "1.1",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Test::More" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "release_status" : "stable",' >> META_new.json
	$(NOECHO) $(ECHO) '   "version" : "1.3"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1.3">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Minimalist stored procedure mapper based on LedgerSMB'\''s DBObject</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Chris Travers &lt;chris.travers@gmail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="PGObject::" VERSION="1.1" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/PGObject/Simple.pm blib/lib/PGObject/Simple.pm 
